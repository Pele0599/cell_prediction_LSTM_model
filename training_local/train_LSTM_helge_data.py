...
#Script used for training a new model 
...
device = torch.device("cpu") #Train it on graphics card or on local CPU
...
save_path = model_config["save_model_path"]
...
FOR NOW JUST USE THE PRELOADED DATA FROM LEON
'''

data_dict = dl.load_data_all_channels(data_config["abs_data_path"])

#%%

# The input data we need for training the model is 
# 1) A voltage profile as the cell is charged and later discharged for each cycle
# 2) Charge on the battery as it is charged to full capacity
# 3) Charge on the battery as it is discharged 
# 4) The cycle life of the battery, for example how many cycles before reaching 
# 80 %
# 5) The charge policy which is an array containing 
# [charge_rate in C, switching_point in SOC for example 0.8C, discharge_rate in C]
...
print("Input shape locxal", num_augment)
...
# Saving model paramaters for the LSTM model inside a .pt format 
