...
THIS SCRIPT IS USED FOR TRAINING AN ALREADY EXISTING MODEL
THE MODEL IS STORED WITH THE SAME NAME 
...
device = torch.device("cpu") #Train it on cpu. If grahpics card available insert "cuda"
...
args = get_args() #GET THE SPECIFIED ARGUMENTS
#%%
model_path = model_config["save_model_path"]
model_abs_path = model_path + model_config["model_name"] + ".pt"          #Load the model which you want to retrain on the data 
...
'''
TODO 
WE LATER WANT TO IMPLEMENT THIS WITH REALTIME ARBIN DATA
FOR NOW JUST USE THE PRELOADED DATA
'''
...
#Load the model from the config file, and train with CPU
model.load_state_dict(torch.load(model_abs_path,map_location=torch.device('cpu')))
...
np.random.seed()
file_name = model_config["model_name"]#"".join([str(np.random.choice(10)) for x in range(10)])
...
pkl.dump(results, open(os.path.join(save_path, file_name + ".pkl"), "wb")) #We dump the results such as RMSE, etc

torch.save(model.state_dict(), oj(save_path, file_name + ".pt")) #The parameters of the neural network model are saved
print("Saved")
